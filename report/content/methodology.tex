...
Alternative muscles that would be interesting to test are the back muscles, namely \textit{Trapezius} \& \textit{Latissimus Dorsi} as proposed in \cite{KeunTaeKim2021}.
...
The paper \cite{KeunTaeKim2021} proposes the \gls{SHAP} \cite{shap} as its main method of dataset creation, an alternative would be the ``Sollerman Hand  Function Test''  \cite{sollerman}.
...
The paper explains that the sEMG data in the dataset has been subject to a pre-processing step using a $10$ to $500Hz$ Buttersworth bandpass filter, and in order to remove power-line noise a $60Hz$ notch filter was used.
%TODO: What is powerline noise!
...
\subsection{Cleaning of Recorded Dataset}
...
Furthermore, software that was able to remove, merge and sub-sample markers was used to clean the labels.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        %\caption{Created motion capture glove.}
    \end{subfigure}
    \caption{Created \& anatomical reference design for the created simulated prosthetic hand.}
